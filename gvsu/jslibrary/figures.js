...
function Axes(draw_yorigin_tick) {
	...
	this.drawHTicks = function () {
		for (var x = this.ticks[0][0]; x <= this.ticks[0][2]; x += this.ticks[0][1]) {
	...
	this.drawHLabels = function () {
		for (var x = this.labels[0][0]; x <= this.labels[0][2]; x += this.labels[0][1]) {
			if (Math.abs(x) < 1e-10) continue;
...
function Slider(rx, y, sx, update) {
	...
	this.drawTicks = function () {
		for (var x = this.ticks[0]; x <= this.ticks[2]; x += this.ticks[1]) {
			this.ctx.save();
	...
	this.drawLabels = function () {
		for (var x = this.labels[0]; x <= this.labels[2]; x += this.labels[1]) {
