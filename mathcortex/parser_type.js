...
function parse_m(code_inp, is_command)
{
	try
	{
		Init(code_inp);
...
var types = [ "reserved", "bool", "real", "matrix", "string", "function", "functionptr"];
...
function GetMatrix()
{
	...
	while( 1)
	{
		...
		{
			Error_p("Matrix in matrix is not allowed yet.");
		}
...
function AddSubOp(top_type, op)
{
	...
	else if (top_type == 4 && reg0_type == 4 && op != '-')
	{
		Emitln( "asm_pop_str_concat();" );
		rvalue[rvalue_pos] = true;
		return 4;
	}
...
function MulDivOpElm(top_type, op)
{
	...
	if (op == './')
	{
		Match('.');
		Match('/');
		instruction = 'asm_pop_elm_div';
	}
	else if (op == '.*')
	{
		Match('.');
		Match('*');
		instruction = 'asm_pop_elm_mul';
	}
...
function BoolOp(top_type, op)
{
	...
	if (op=='||')
	{
		Match('|');
		Match('|');
		prefix = 'asm_pop_or_bool';
	}
	else if ( op == '&&')
	{
		Match('&');
		Match('&');
		prefix = 'asm_pop_and_bool';
	}
	else if ( op == '!')
	{
		Match('!');
...
function RelOp(top_type, op)
{
	...
	if (reg0_type ==4 && top_type==4)
	{
		if (op=='==')
		{
			prefix = 'asm_pop_str_eq';
		}
		else if ( op == '!=')
		{
			prefix = 'asm_pop_str_neq';
		}
		else
		{
	...
	else if(reg0_type ==3 && top_type==3)
	{
		if (op=='==')
		{
			prefix = 'asm_pop_matrix_eq';
		}
		else if ( op == '!=')
		{
			prefix = 'asm_pop_matrix_neq';
		}
		else
		{
...
function Factor()
{
	...
	else if (Look == '"')
	{
		type = 4;
		Match('"');
		Emitln("asm_reg0 = \"" + add_slashes(GetString()) + "\";");
		Match('"');
	}
	else
	{
