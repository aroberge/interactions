...
var pages = new Array( /*new NewPage("new 1","",-1, false, false,false)*/);
var curpage=-1;
var new_index = 1;
...
function SelectPage(index)
{
	...
	if(curpage!=-1)
	{
		//pages[curpage].text = $('#code_txt').val( );
		$('#pages_btn'+curpage).attr('class', '');
	}

	//$('#code_txt').val( pages[index].text );
	ace_editor.setSession(pages[index].ace_session);
	ace_editor.focus();
	$('#pages_btn'+index).attr('class', 'tabActiveHeader');
...
function AddNewPage()
{
	pages.push(new NewPage( _("new ")+new_index, "", -1, false, false, false));
	new_index++;
...
function UpdatePagesButtons()
{
	...
	for(var i=startFit;i<visiblePageCount+startFit;i++)
	{
        ...
        var title;
        if (document.getElementById("dirselect") != null) {
			title = $('#dirselect').val() == '/' ? pages[i].name : $('#dirselect').val() + '/' + pages[i].name;
		} else {
			title = pages[i].name;
		}
		html += title + "' onclick='SelectPage(" + i + ")'>" + name;
		//html += pages[i].p_id == -1 ? "<img class='' src='icons/page_white_error.png' style='position:absolute;right:20px;' height='14' width='14' title='File is stored on browser cache with best effort. You can login and store at server for reliable storage'></img>" : "";
		html += "<img class='' src='icons/delete-icon.png' style='position:absolute;right:5px;' height='12' width='12'  onclick='RemovePage(" + i + ")'></img></li>";
...
{
	$.post("php/pages_service.php?cmd=new", { key:555 ,text: '' , name:$('#renameFile').val() , dir_id: dir_id, is_dir:true})
			.done(function(data) {
				if (data=="fail")
				{
...
{
	$.get("php/pages_service.php?cmd=mvfile", { key:555 ,src_file_id: src_id, destpath: destpath, cur_dir_id : cur_dir_id, src_path_id : cur_dir_id})
			.done(function(data) {
				try
				{
...
function savepage(page, dir_id, new_name){
	...
	if( page["p_id"]!=-1)
	{
		$.post("php/pages_service.php?cmd=save", { id: page["p_id"], key:555 ,text: page.text , name:page.name  })
			.done(function(data) {
			if (data=="success")
			{
	...
	else
	{
		...
		$.post("php/pages_service.php?cmd=new", datajson)
			.done(function(data) {
			if (data=="fail")
			{
				status_print("Error could not save to server : " + data);
...
function deletepage(id, dir_id){
	if(id!=-1)
	{
		...
		$.post("php/pages_service.php?cmd=delete", { id: id, key:555, dir_id: dir_id })
			.done(function(data) {
			if (data=="deleted")
			{
...
function changedir( dirname)
{
	...
	xmlhttp.open("GET","php/pages_service.php?cmd=getdir&fullpath=" + encodeURIComponent(dirname), true);
	xmlhttp.send();
	...
	if (xmlhttp.readyState==4 && xmlhttp.status==200)
	{
		...
		default :
			...
			for(var i=0; i < server_pages.length;i++)
			{
				...
				{
					server_pages[i]["name"] = _("new ") + new_index;
...
function loadRootDoc()
{
	...
	{
		xmlhttp.open("GET","php/pages_service.php?cmd=getroot", true);
		xmlhttp.send();
