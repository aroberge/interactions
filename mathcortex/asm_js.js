...
//our only register. for complex types like matrix it stores reference.
var asm_reg0 = 0;
...
///// ADD OPs ////////
function asm_pop_add_real()
{
	asm_sp--;
	var eax = stack[asm_sp];
	
	asm_reg0 = ( eax+ asm_reg0);
}

function asm_pop_add_rm() // real matrix add
{
	asm_sp--;
	var eax = stack[asm_sp];
	
	var new_m = numeric.clone(asm_reg0);
	asm_util_matrix_map(new_m, function(x){ return x+eax;});
	
	asm_reg0 = new_m;
}

function asm_pop_add_mr() // real matrix add
{
	asm_sp--;
	var eax = stack[asm_sp];
	
	var new_m = numeric.clone(eax);
	asm_util_matrix_map(new_m, function(x){ return x+asm_reg0;});
	
	asm_reg0 = new_m;
}

function asm_pop_add_mm() // real matrix add
{
	asm_sp--;
	//var eax = stack[asm_sp].dup();
	var eax = stack[asm_sp];	
	
	//var r = eax.add(asm_reg0); 
	if (eax.length != asm_reg0.length || eax[0].length != asm_reg0[0].length)
	{
		Error_run('Matrix size mismatch.');
	}
	var r = numeric.add(eax,asm_reg0); 
	/*if (r==null)
	{
		Error_run('Matrix size mismatch.');
	}*/
...
function asm_pop_sub_mm() // real matrix sub
{
	asm_sp--;
	//var eax = stack[asm_sp].dup();
	var eax = stack[asm_sp];
	
	//var r = eax.subtract(asm_reg0); 
	if (eax.length != asm_reg0.length || eax[0].length != asm_reg0[0].length)
	{
		Error_run('Matrix size mismatch.');
	}
	var r = numeric.sub(eax,asm_reg0); 
	/*if (r==null)
	{
		Error_run('Matrix size mismatch.');
	}*/
...
function asm_pop_sub_rm() // scalar matrix ops
{
	asm_sp--;
	var eax = stack[asm_sp];
	
	var new_m = numeric.clone(asm_reg0);
	asm_util_matrix_map(new_m, function(x){ return eax-x;});
	
	asm_reg0 = new_m;
}

function asm_pop_sub_mr() // scalar matrix ops
{
	asm_sp--;
	var eax = stack[asm_sp];
...
function asm_pop_mul_rm() // scalar matrix ops
{
	asm_sp--;
	var eax = stack[asm_sp];
...
function asm_pop_mul_mr() // scalar matrix ops
{
	asm_sp--;
	var eax = stack[asm_sp];
...
function asm_pop_mul_mm() // matrix matrix mmul
{
	asm_sp--;
	//var eax = stack[asm_sp].dup();
	var eax = stack[asm_sp];	
	//var r = eax.multiply(asm_reg0);
	if (eax[0].length != asm_reg0.length)
	{
		Error_run('Matrix size mismatch.');
	}
	var r = numeric.dot(eax,asm_reg0);
...
function asm_pop_div_real()
{
 ...
 function asm_pop_div_mr() // scalar matrix ops
{
	asm_sp--;
	var eax = stack[asm_sp];
...
function asm_pop_elm_mul()
{
	...
	var r = numeric.mul(eax,asm_reg0);
	
	asm_reg0 = r;
}

function asm_pop_elm_div()
{
	asm_sp--;
	var eax = stack[asm_sp];
	if (eax.length != asm_reg0.length || eax[0].length != asm_reg0[0].length)
	...
	var r = numeric.div(eax,asm_reg0);
	
	asm_reg0 = r;
}

///// BOOL OPs ////////
function asm_pop_and_bool()
{
	asm_sp--;
	var eax = stack[asm_sp];
...
function asm_pop_eq()
{
	...
	var r = asm_reg0==eax ? 1 : 0; 
	asm_reg0 = r;
}

function asm_pop_neq()
{
	asm_sp--;
	var eax = stack[asm_sp];
...
function asm_pop_matrix_eq()
{
	asm_sp--;
	var eax = stack[asm_sp];
	
	var r = asm_matrix_same(eax, asm_reg0) ? 1 : 0; 
	asm_reg0 = r;
}

function asm_pop_matrix_neq()
{
	asm_sp--;
	var eax = stack[asm_sp];
